...
class DataFetcher:
    ...
    def execute_discovery_queries(self, strategy: Dict[str, Any]) -> Tuple[int, List[int]]:
        ...
        count_sql = self._sanitize_query(strategy['count_query'])
        ids_sql = self._sanitize_query(strategy['id_list_query'])
    ...
    def _sanitize_query(self, sql: str) -> str:
        """
        Fixes common LLM SQL syntax errors.
        Example: Replaces text accessor ->> with jsonb accessor -> when using containment operator @>
        """
        import re
        # Fix: obj->>'keywords' @> ... -> obj->'keywords' @> ...
        # logic: if we see ->>'key' followed by @>, we change it to ->'key'
        # Captures: 1=key
        pattern = r"->>\s*'([^']+)'\s*@>"
        fixed_sql = re.sub(pattern, r"->'\1' @>", sql)

        if fixed_sql != sql:
            logger.info(f"[SQL FIX] Sanitized query: {fixed_sql}")
